...
internal static class HydrologyChecks
{
    ...
    public static void Run(TerrainPatternSettings terrainAsset, HydrologyFeatureSettings asset)
    {
        ...
        var elevationReader = new ElevationPatternMapReader(grid,cache,new ElevationPatternSettingsData(terrainSettings));
        var cacheReader = new ClimatePatternMapReader(grid,cache,elevationReader,new ClimateSettings());
        var terrainBuilder = new TerrainPatternTileBuilder(grid,terrainSettings,elevationReader,cacheReader);
        var terrainReader = new TerrainPatternMapReader(grid,cache,terrainBuilder);
        var cachedDrawer = new HydrologyPatternDrawer(grid,settings,terrainReader,cache.WaterBrushes,cacheReader);
        var cacheKey = keys[0];
