...
## DEC-013 — 다중 Entity 배치의 부분 성공

- **상태:** Active
- **결정:** 다중 Entity 배치는 부분 성공을 허용한다. 개체별 실패 후에도 나머지 배치를 계속하며, 성공한 개체를 일괄 취소하지 않는다.
- **집계·안내:** 실제 등록 완료를 기준으로 성공·실패 개수를 집계하고 로그로 출력한다. 실패 원인도 로그로 남긴다. 실패한 개체의 불완전한 등록은 정리한다.
- **기존 결정과의 관계:** DEC-012의 Entity 이력 제외 및 실패한 개별 작업의 부수 World 변경 복구 원칙은 유지한다. 구현 상태는 ROADMAP의 프로토타입 최종 정리 3단계에서 관리한다.

## DEC-012 — 직접 World 편집의 Undo/Redo 범위

- **상태:** Active
- **결정:** Undo/Redo는 사용자가 World 도구로 직접 수행한 편집만 대상으로 한다. 현재 Terraform·Road 편집이 해당한다. Entity 생성·배치·삭제와 Entity로 인해 발생한 지형·Road 등 World 변경은 이력에 포함하지 않는다. Entity 작업과 그 부수 변경을 복합 Undo 기록으로 만들지 않는다.
- **실패 복구:** 이력 제외는 실패 복구 제외를 뜻하지 않는다. Entity 배치 작업이 실패하면 그 작업에서 수행한 World 변경도 복구해야 한다. 실패 복구는 사용자 Undo/Redo와 별개의 작업 일관성 책임이다.
- **이력 폐기:** Undo 범위 밖의 작업이 World를 실제 변경하여 성공하면 기존 World Undo/Redo 이력을 모두 폐기한다. 기존 기록과 변경 영역이 겹치는지에 관계없이 적용한다. Entity 작업을 이력에 추가하지 않는다.
- **폐기 시점:** 부수 World 변경을 포함한 작업 전체가 성공한 뒤 폐기한다. World 변경이 없는 Entity 작업, 실제 변경 없는 작업, 취소 또는 실패 후 복구된 작업은 이력을 폐기하지 않는다. Undo/Redo 실행 자체는 범위 밖의 새 작업으로 취급하지 않는다.
- **Streaming 경계:** Undo/Redo의 다음 기록에 포함된 모든 Chunk 데이터가 로드되어 있을 때만 실행한다. 미로드 기록은 삭제하거나 건너뛰지 않고 실행을 보류하며, 재로드되면 다시 허용한다. 렌더링·시뮬레이션 활성 여부는 기준이 아니다. 여러 Chunk 기록의 부분 적용 및 Undo 전용 자동 로드는 하지 않는다.
- **구조 기준:** 상태 소유권과 전환 책임을 명확히 하고, 편집 판단과 UI·Preview 표시를 분리한다. 최적화를 위해 여러 Controller가 공유하는 가변 캐시나 불필요한 Manager·Service·전역 이벤트 시스템을 추가하지 않는다. 구체 클래스 분리와 이력 충돌 구현은 후속 작업에서 확정한다.
- **영향 시스템:** Editing, Entity 배치, World 편집 이력, UI의 실행 결과 표시. 기존 UI 배치·도구·선택 규칙은 DEC-009 / DEC-010 / DEC-011을 유지한다. 구현 및 사용자 확인 상태는 ROADMAP.md를 따른다.
