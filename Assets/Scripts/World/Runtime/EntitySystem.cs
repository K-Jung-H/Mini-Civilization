...
namespace MiniCivilization.World.Runtime
{
    public sealed class EntitySystem
    {
        ...
        private readonly Dictionary<EntityId, EntityRuntime> entitiesById = new();
        ...
        private readonly Dictionary<CellCoordinate, EntityId> terrainAnchorOwners = new();
        private readonly HashSet<EntityRuntime> failedReleases = new();
        private readonly HashSet<CellColumnCoordinate> terrainAnchorColumns = new();
        private readonly Dictionary<ChunkCoordinate, List<EntityRuntime>>
        ...
        private readonly List<EntityRuntime> tickBuffer = new();
        private readonly List<TickEntry> tickEntries = new();
        ...
        private readonly Dictionary<EntityId, WayMovementPlan>
            activeWayMoves = new();
        private readonly Stack<EntityRuntime> availableRuntimeSlots = new();
        ...
        public void CopyEntitiesTo(List<EntityRuntime> target)
        {
            ...
            target.Sort((left, right) => left.Id.CompareTo(right.Id));
        }
        ...
        internal bool CanRestorePersistentState(EntityPersistentState state)
        {
            ...
            try
            {
                ...
                var referencedChunks = ResolveReferencedChunks(
                    entity,
                    state.ActiveWayMove);
            ...
            {
                entity.PrepareRelease();
                entity.CompleteRelease();
            }
        }
            ...
            EntityPersistentState state)
        {
            ...
            var entity = CreateRuntime(data);
            var worldAdded = false;
            try
            {
                RestoreEntityRuntimeState(entity, state);
                ...
                if (entity.FSM is DynamicEntityFSM dynamicEntity
                ...
                if (state.ActiveWayMove != null)
                {
                    ...
                    RefreshEntityChunkReferences(entity);
                }
            ...
            {
                if (entitiesById.TryGetValue(state.Id, out var registered)
                ...
                {
                    ReleaseUnregisteredRuntime(entity);
                }

                if (worldAdded)
                {
                    world.RemoveEntity(state.Id);
                }
                throw;
            }

            if (entity.FSM is BuildingEntityFSM && runtime.WayPointGraph != null)
            {
            ...
            return PublishChange(
                ...
                wayTopologyChanged: entity.FSM is BuildingEntityFSM);
        }
            ...
            List<EntityPersistentState> target)
        {
            ...
            for (var index = 0; index < tickBuffer.Count; index++)
            {
                ...
                topologyChanged |= entity.FSM is BuildingEntityFSM;
                RemoveRuntimeEntity(entity);
        ...
        internal void CopyEntitiesInPreparedChunksTo(List<EntityRuntime> target)
        {
            if (target == null)
            {
                throw new ArgumentNullException(nameof(target));
        ...
        public void CopyMovingEntitiesTo(List<DynamicEntityFSM> target)
        {
            if (target == null)
            {
                throw new ArgumentNullException(nameof(target));
            ...
            {
                if (entitiesById.TryGetValue(id, out var entity)
                    && entity.FSM is DynamicEntityFSM dynamicEntity
                    && dynamicEntity.IsMoving)
                {
        ...
        internal void Tick(float deltaTime)
        {
            ...
            foreach (var pair in runtime.ChunkRuntimes)
            {
                        ...
                        out var chunkEntities))
                {
                    ...
                    {
                        var entity = chunkEntities[index];
                        tickEntries.Add(new TickEntry(
                            entity,
                            entity.Id,
                            entity.BindingVersion));
                    }
                }
            }

            tickEntries.Sort(CompareTickEntries);
            for (var index = 0; index < tickEntries.Count; index++)
            {
                var entry = tickEntries[index];
                var entity = entry.Runtime;
                if (!entity.IsBinding(entry.Id, entry.BindingVersion)
                    || !entitiesById.TryGetValue(entry.Id, out var registered)
                    || !ReferenceEquals(registered, entity)
                ...
                if (entity.IsBinding(entry.Id, entry.BindingVersion)
                    && (previousActivity != entity.Activity
                        || previousPhase != entity.ActivityPhase
                        || previousTarget != entity.InteractionTargetId)
                    && entitiesById.TryGetValue(entry.Id, out var current)
                    && ReferenceEquals(current, entity))
                {
                    PresentationChanged?.Invoke(entry.Id);
                }
            }
        }
        ...
        public EntityChangeSet Add(EntityData data)
        {
            ...
            {
                if (entitiesById.TryGetValue(data.Id, out var registered)
                    && ReferenceEquals(registered, entity))
                {
                    RemoveRuntimeEntity(entity);
                }
            ...
            EntityData data)
        {
            ...
            if (registry.Create(data).FSM is not BuildingEntityFSM building)
            {
        ...
        public EntityChangeSet Remove(EntityId id)
        {
            ...
            var wasBuilding = entity.FSM is BuildingEntityFSM;
            RemoveRuntimeEntity(entity);
            world.RemoveEntity(id);
            if (wasBuilding && runtime.WayPointGraph != null)
            {
                runtime.RebuildWayPointGraph();
            }
            ...
            return PublishChange(
                ...
                NoEntityIds,
                affectedCells,
                wayTopologyChanged: wasBuilding);
        }

        internal bool TryBeginMove(
            DynamicEntityFSM entity,
            ...
            EntityMoveType moveType)
        {
            ...
            if (!entitiesById.TryGetValue(entity.Id, out var registered)
                || !ReferenceEquals(registered.FSM, entity))
            {
                throw new InvalidOperationException(
            ...
            if (!world.Contains(destination.X, destination.Y, destination.Z)
                || !world.IsChunkLoaded(ToChunk(current))
                || !world.IsChunkLoaded(ToChunk(destination)))
            {
                return false;
            }
            ...
            if (wayPlan != null
                && !AreWayPointChunksLoaded(wayPlan.GraphPositions, entity.Id))
            {
                return false;
            }
            ...
            RefreshEntityChunkReferences(registered);
            movingEntityIds.Add(entity.Id);
            PresentationChanged?.Invoke(entity.Id);
            return true;
        }

        internal EntityChangeSet CompleteMove(DynamicEntityFSM entity)
        {
            if (entity == null)
            {
                throw new ArgumentNullException(nameof(entity));
            ...
            RemoveEntityChunkReferences(registered);
            if (!previousChunk.Equals(nextChunk) && entity.RequiresTick)
            {
                RemoveTickEntity(registered, previousChunk);
            }
            ...
            AddEntityChunkReferences(registered);
            if (!previousChunk.Equals(nextChunk) && entity.RequiresTick)
            {
                AddTickEntity(registered, nextChunk);
            }
            ...
            CellCoordinate nextCell)
        {
            if (entity == null
                ...
                || !world.IsChunkLoaded(ToChunk(currentCell))
                || !world.IsChunkLoaded(ToChunk(nextCell)))
            {
                return false;
            }
            ...
            WorldWayPointGraph graph)
        {
            ...
            foreach (var pair in buildingWayLocations)
            {
                if (!entitiesById.ContainsKey(pair.Key)
                    ...
                    || buildingEntity.FSM is not BuildingEntityFSM building
            ...
            foreach (var entity in entitiesById.Values)
            {
                if (entity.FSM is not DynamicEntityFSM
        ...
        private EntityPersistentState CapturePersistentState(EntityRuntime entity)
        {
        ...
        {
            var positions = state.GraphPositions;
            ...
            EntityPersistentState state)
        {
            ...
            if (state.ActiveWayMove != null
                && entity.FSM is not DynamicEntityFSM)
            {
                throw new InvalidOperationException(
        ...
        private void AddRuntimeEntity(EntityRuntime entity)
        {
            if (entity == null)
            {
                throw new ArgumentNullException(nameof(entity));
            ...
            try
            {
                if (entity.FSM is BuildingEntityFSM building)
                {
            ...
            catch
            {
                ...
                if (entity.FSM is BuildingEntityFSM building)
                {
                    RemoveBuilding(building, requireExisting: false);
                }
                else
                {
        ...
        private void RemoveRuntimeEntity(EntityRuntime entity)
        {
            if (entity == null
                || !entity.IsBound
                || !entitiesById.TryGetValue(entity.Id, out var registered)
                || !ReferenceEquals(registered, entity))
            {
                throw new InvalidOperationException(
                    "Entity Runtime is not the registered binding being removed.");
            }
        ...
        private void ReleaseUnregisteredRuntime(EntityRuntime entity)
        {
            if (entity == null || !entity.IsBound || failedReleases.Contains(entity))
            {
                return;
            }
        ...
        private void ReleaseRuntimeSlot(EntityRuntime entity)
        {
            try
            {
                entity.PrepareRelease();
            ...
            {
                failedReleases.Add(entity);
                UnityEngine.Debug.LogException(exception);
            }

            try
            {
                NotifyEntityReleasing(entity);
            }
            catch (Exception exception)
            {
                failedReleases.Add(entity);
            ...
            if (failedReleases.Contains(entity)) return;
            entity.CompleteRelease();
            availableRuntimeSlots.Push(entity);
        }

        private void NotifyEntityReleasing(EntityRuntime entity)
        {
            var handlers = EntityReleasing;
            if (handlers == null)
            {
                return;
            }
            ...
            foreach (Action<EntityRuntime> handler in handlers.GetInvocationList())
            {
                try
                {
                    handler(entity);
            ...
            if (failures != null)
            {
                throw new AggregateException(
                    "Entity View release failed; Runtime cannot be reused.", failures);
            }
        }

        private EntityRuntime CreateRuntime(EntityData data)
        {
            var slot = availableRuntimeSlots.Count > 0
                ? availableRuntimeSlots.Pop()
                : null;
            try
            {
                return registry.Create(data, slot);
            }
            catch
            {
                if (slot != null && !slot.IsBound)
                {
                    availableRuntimeSlots.Push(slot);
                }
        ...
        private void AddTickEntity(EntityRuntime entity, ChunkCoordinate coordinate)
        {
            if (!tickEntitiesByChunk.TryGetValue(coordinate, out var entities))
            {
                entities = new List<EntityRuntime>();
        ...
        private void AddEntityChunkReferences(EntityRuntime entity)
        {
        ...
        private void RefreshEntityChunkReferences(EntityRuntime entity)
        {
            RemoveEntityChunkReferences(entity);
            AddEntityChunkReferences(entity);
        }
        ...
        private ChunkCoordinate[] ResolveReferencedChunks(
            EntityRuntime entity,
            WayMovementPlanPersistentState restoredWayMove = null)
        {
            ...
            else
            {
                ...
                {
                    RequireWorldCell(moving.MoveFrom, entity.Id);
                    RequireWorldCell(moving.MoveTo, entity.Id);
                    chunks.Add(ToChunk(moving.MoveFrom));
                    chunks.Add(ToChunk(moving.MoveTo));
                }
            }

            if (activeWayMoves.TryGetValue(entity.Id, out var activeWayMove))
            {
                AddWayPointChunks(chunks, activeWayMove.GraphPositions, entity.Id);
            }
            else if (restoredWayMove != null)
            {
                AddWayPointChunks(chunks, restoredWayMove.GraphPositions, entity.Id);
            }
        ...
        private void AddWayPointChunks(
            ISet<ChunkCoordinate> chunks,
            IReadOnlyList<UnityEngine.Vector3> positions,
            EntityId id)
        {
            for (var index = 0; index < positions.Count; index++)
            {
                ...
                if (!float.IsFinite(position.x) || !float.IsFinite(position.y)
                    ...
                    throw new InvalidOperationException($"Entity {id} has a non-finite movement path.");
                var x = UnityEngine.Mathf.FloorToInt(position.x / world.CellSize);
                var z = UnityEngine.Mathf.FloorToInt(position.z / world.CellSize);
                if (!world.ContainsColumn(x, z))
                {
                    throw new InvalidOperationException(
                        $"Entity {id} movement path is outside the world at ({x}, {z}).");
                }

                chunks.Add(WorldCoordinateUtility.ToChunk(x, z, world.ChunkSizeX));
                if (index > 0)
                    AddSegmentChunks(chunks, positions[index - 1], position);
            }
        }

        private void AddSegmentChunks(ISet<ChunkCoordinate> chunks,
            UnityEngine.Vector3 from, UnityEngine.Vector3 to)
        {
            var size = (double)world.CellSize * world.ChunkSizeX;
            var x = (int)Math.Floor(from.x / size);
            var z = (int)Math.Floor(from.z / size);
            var endX = (int)Math.Floor(to.x / size);
            var endZ = (int)Math.Floor(to.z / size);
            var dx = (double)to.x - from.x;
            var dz = (double)to.z - from.z;
            var sx = Math.Sign(dx);
            var sz = Math.Sign(dz);
            var tx = sx == 0 ? double.PositiveInfinity
                : ((sx > 0 ? x + 1 : x) * size - from.x) / dx;
            var tz = sz == 0 ? double.PositiveInfinity
                : ((sz > 0 ? z + 1 : z) * size - from.z) / dz;
            while (x != endX || z != endZ)
            {
                if (x != endX && (z == endZ || tx < tz))
                {
                    x += sx;
                    tx += size / Math.Abs(dx);
                }
        ...
        private bool AreWayPointChunksLoaded(
            IReadOnlyList<UnityEngine.Vector3> positions,
            EntityId id)
        {
            var chunks = new HashSet<ChunkCoordinate>();
            AddWayPointChunks(chunks, positions, id);
        ...
        private void AddBuilding(BuildingEntityFSM building)
        {
            var layout = building.Layout
                ?? throw new InvalidOperationException(
            ...
            for (var index = 0; index < anchors.Length; index++)
            {
                ...
                terrainAnchorOwners.Add(anchors[index], building.Id);
        ...
        {
            var layout = building.Layout;
            if (layout == null && !requireExisting) return;
            for (var index = 0; index < layout.BuildingCells.Count; index++)
            {
                var coordinate = layout.ToWorld(
                    building.Data,
                    layout.BuildingCells[index].LocalOffset);
                var removed = buildingCells.TryGetValue(coordinate, out var owner)
                    && owner.BuildingId == building.Id;
                if (removed) buildingCells.Remove(coordinate);
                if (requireExisting && !removed)
                {
                    throw new InvalidOperationException(
                        $"Building {building.Id} is not indexed at Cell {coordinate}.");
                }
            ...
            for (var index = 0; index < layout.TerrainAnchorCells.Count; index++)
            {
                var coordinate = layout.ToWorld(
                    building.Data,
                    layout.TerrainAnchorCells[index].LocalOffset);
                if (!terrainAnchorOwners.TryGetValue(coordinate, out var anchorOwner)
                    || anchorOwner != building.Id)
                {
                    if (requireExisting)
                        throw new InvalidOperationException($"Building {building.Id} does not own Terrain Anchor {coordinate}.");
                    continue;
                }
                terrainAnchorOwners.Remove(coordinate);
        ...
        private void RemoveEntityFromCell(
            EntityId id,
            CellCoordinate coordinate,
            bool requireExisting = true)
        {
        ...
        private CellCoordinate[] GetIndexedCells(EntityRuntime entity)
        {
            if (entity.FSM is not BuildingEntityFSM building)
            {
        ...
        private readonly struct TickEntry
        {
            public TickEntry(
            ...
            public EntityRuntime Runtime { get; }
            public EntityId Id { get; }
            public uint BindingVersion { get; }
        }

        private static int CompareTickEntries(TickEntry left, TickEntry right) =>
        ...
        private static int CompareEntities(EntityRuntime left, EntityRuntime right) =>
            left.Id.CompareTo(right.Id);
